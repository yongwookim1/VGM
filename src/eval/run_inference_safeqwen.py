"""Run SafeQwen2.5-VL inference on the processed test split."""
...
from src.models.safeqwen.dataset import SYSTEM_PROMPT, _resize_frame, sample_frames_from_video
...
def load_model_and_processor(args):
    logger.info("Loading processor from %s", args.processor_name)
...
def generate_prediction(model, processor, sample, args):
    ...
    safety_pred = (
        ...
        if outputs.img_safety_logits is not None
        else None
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run SafeQwen inference on the test set")
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--base_model", default="models/SafeQwen2.5-VL-7B")
    parser.add_argument("--processor_name", default="models/SafeQwen2.5-VL-7B")
    ...
    for sample in tqdm(test_data, desc="SafeQwen inference"):
