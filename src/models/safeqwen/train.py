"""Training entrypoint for SafeQwen on the unified video safety dataset."""
...
from src.models.safeqwen.modeling import load_safeqwen
...
class ModelArguments:
    model_name_or_path: str = field(default="models/SafeQwen2.5-VL-7B")
    processor_name: str = field(default="models/SafeQwen2.5-VL-7B")
...
class VideoSafetyCollator:
    """Collate SafeQwen video/text samples into padded batches."""

    def __init__(self, pad_token_id: int = 151643, ignore_index: int = -100, max_length: int | None = None):
    ...
    def __call__(self, batch: list[dict]) -> dict | None:
        ...
        for idx, sample in enumerate(batch):
            ...
            input_ids[idx, :seq_len] = sample["input_ids"][:seq_len]
            attention_mask[idx, :seq_len] = sample["attention_mask"][:seq_len]
            labels[idx, :seq_len] = sample["labels"][:seq_len]
        ...
        pvv_list = [sample["pixel_values_videos"] for sample in batch if "pixel_values_videos" in sample]
        vgt_list = [sample["video_grid_thw"] for sample in batch if "video_grid_thw" in sample]
        if pvv_list:
